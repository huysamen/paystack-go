...
func TestSubmitAddressResponse_FieldByFieldValidation(t *testing.T) {
	...
	// Handle phone field - empty string in JSON becomes empty or invalid
	if rawCustomer["phone"] == nil || rawCustomer["phone"] == "" {
		if !response.Data.Customer.Phone.Valid {
			assert.False(t, response.Data.Customer.Phone.Valid, "phone should be invalid")
		} else {
			assert.Equal(t, "", response.Data.Customer.Phone.String(), "phone should be empty string")
		}
	} else {
