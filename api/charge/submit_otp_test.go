...
func TestSubmitOTPResponse_FieldByFieldValidation(t *testing.T) {
	// Read the submit_otp_200.json file
	responseFilePath := filepath.Join("..", "..", "resources", "examples", "responses", "charge", "submit_otp_200.json")
	responseData, err := os.ReadFile(responseFilePath)
	require.NoError(t, err, "failed to read submit_otp_200.json")
	...
	// Deserialize into the SubmitOTPResponse struct
	var response SubmitOTPResponse
	err = json.Unmarshal(responseData, &response)
	require.NoError(t, err, "failed to unmarshal into SubmitOTPResponse struct")
	...
	// Validate data object fields (this response has additional fields compared to create)
	...
	var roundTripResponse SubmitOTPResponse
