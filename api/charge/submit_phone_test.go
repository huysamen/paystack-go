...
func TestSubmitPhoneResponse_FieldByFieldValidation(t *testing.T) {
	// Read the submit_phone_200.json file
	responseFilePath := filepath.Join("..", "..", "resources", "examples", "responses", "charge", "submit_phone_200.json")
	responseData, err := os.ReadFile(responseFilePath)
	require.NoError(t, err, "failed to read submit_phone_200.json")
	...
	// Deserialize into the SubmitPhoneResponse struct
	var response SubmitPhoneResponse
	err = json.Unmarshal(responseData, &response)
	require.NoError(t, err, "failed to unmarshal into SubmitPhoneResponse struct")
	...
	var roundTripResponse SubmitPhoneResponse
