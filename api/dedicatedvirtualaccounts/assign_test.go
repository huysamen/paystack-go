...
func TestAssignResponse_FieldByFieldValidation(t *testing.T) {
	// Read the assign_200.json file
	responseFilePath := filepath.Join("..", "..", "resources", "examples", "responses", "dedicatedvirtualaccounts", "assign_200.json")
	responseData, err := os.ReadFile(responseFilePath)
	require.NoError(t, err, "failed to read assign_200.json")
	...
	// Deserialize into the AssignDedicatedVirtualAccountResponse struct
	var response AssignDedicatedVirtualAccountResponse
	err = json.Unmarshal(responseData, &response)
	require.NoError(t, err, "failed to unmarshal into AssignDedicatedVirtualAccountResponse struct")
	...
	assert.False(t, hasData, "data field should not exist in assign response")

	// Test round-trip serialization
	serialized, err := json.Marshal(response)
	require.NoError(t, err, "failed to marshal response back to JSON")

	var roundTripResponse AssignDedicatedVirtualAccountResponse
