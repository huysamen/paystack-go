...
func TestArchiveResponse_JSONDeserialization(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "successful archive response",
			responseFile:    "archive_200.json",
			expectedStatus:  true,
			expectedMessage: "Payment request has been archived",
		},
	}
...
func TestFinalizeResponse_JSONDeserialization(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "successful finalize response",
			responseFile:    "finalize_200.json",
			expectedStatus:  true,
			expectedMessage: "Payment request finalized",
		},
	}
...
func TestSendNotificationResponse_JSONDeserialization(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "successful send notification response",
			responseFile:    "send_notification_200.json",
			expectedStatus:  true,
			expectedMessage: "Notification sent",
		},
	}
...
func TestVerifyResponse_JSONDeserialization(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "successful verify response",
			responseFile:    "verify_200.json",
			expectedStatus:  true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify the data structure (similar to fetch response)
			assert.NotNil(t, response.Data, "data should not be nil")
			assert.Greater(t, response.Data.ID.Int64(), int64(0), "ID should be greater than 0")
			assert.NotEmpty(t, response.Data.RequestCode.String(), "request code should not be empty")
		})
	}
}
